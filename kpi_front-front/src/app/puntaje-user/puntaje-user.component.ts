...
export class PuntajeUserComponent implements OnInit{
  ...
  async findByDate(monthString: string): Promise<void> {
    ...
    // First call
    this.puntajes = await this.http.post<any>(url, { mes, anio }, {
      headers: {
    ...
    // Second call to ensure data is updated
    this.puntajes = await this.http.post<any>(url, { mes, anio }, {
      headers: {
        'Cache-Control': 'no-cache, no-store, must-revalidate',
        'Pragma': 'no-cache',
  ...
  async showReport(item: any, id: number, comentario: string, calificacionLider: number, date: string) {
    // Forzar una recarga de los datos antes de generar el PDF
    await this.findByDate(date); // Asegura que los datos estén actualizados
  
    // Verifica si los datos se actualizaron correctamente
    // if (!this.puntajes) {
    //   Swal.fire('Error', 'No se pudieron cargar los datos actualizados', 'error');
    //   return;
    // }
  
    // // Busca el empleado específico en los puntajes actualizados
    // const empleadoActualizado = this.puntajes.find((p: any) => p.empleado.id === id);
    // if (!empleadoActualizado) {
    //   Swal.fire('Error', 'No se encontró el empleado en los datos actualizados', 'error');
    //   return;
    // }
